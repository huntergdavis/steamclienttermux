...
| BVB shared descriptor journal | E125 measured about 529 synchronous update-template RPCs and 72.9 ms blocked in them per benchmark-scene present. E126 removed opcode 112 and reduced combined descriptor blocking 13.8%, from 162.4 to 139.9 ms/present, but average FPS remained 2.0. | Keep the exact final-process-only selector. Because opcode 68 interleaves with updates, E126 averaged only 1.5 updates per drain. E127 must move ordered native allocation results and updates onto one shared request/completion transport; a larger journal alone cannot solve it. |
