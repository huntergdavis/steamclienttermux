...
| FEX persistent code cache | Upstream describes the experimental cache as reducing repeated JIT work and stutter; the installed ARM64EC module exposes both required controls. Current MaxInst is already the upstream default, and fixed L1/L2 caches remain the latency-oriented choice. | Stage default-off final-game-only selector, warm it once, then run three cooled recorded passes. Compare against the post-RakNet 32.667-FPS control mean and follow with a reverse control if the gain is decision-sized. |
...
| FEX persistent code cache | Staged default-off; host containment and cache-directory tests pass, tablet result pending. | Promote only after cached runs improve a cooled series without correctness, storage, or stutter regressions; otherwise keep it as an opt-in startup/stutter tool. |
