...
| Bionic/system-Vulkan host | Direct enumeration, glibc/Bionic parity, native command submission, surface query, and a six-image swapchain clear/present/consumer-readback loop now pass with all 4,096 pixels verified. | Continue as a separate architecture project. Next add the dedicated visible `SurfaceView` host and bridge its lifecycle; game-facing dispatch and DXVK integration still precede any Tomb Raider FPS A/B. |
