...
| Bionic/system-Vulkan host | Direct enumeration, glibc/Bionic parity, native commands, pixel-verified offscreen presentation, and a standalone immersive visibly magenta Android `NativeActivity` now pass. | Continue as a separate architecture project. Next add explicit visible-window lifecycle/status handoff through the bridge; shared-UID/game-facing dispatch and DXVK integration still precede any Tomb Raider FPS A/B. |
