...
| BVB descriptor transaction / completion ring | E127 fused updates plus each real allocation result, raising average FPS from 2.0 to 2.3 and cutting scene descriptor blocking 27.1% to 102.1 ms/present. The remaining 526 opcode-123 request/reply transactions per present alone cost 100.3 ms. | Preserve E127 ordering and typed allocation outcomes, but move requests/completions onto a bounded shared ring or a pool-epoch-aware lease cache. The per-allocation socket round trip is now the measured primary target. |
