...
| Steam `IPC:CServiceEng` on CPU0 | Implemented but not yet promoted: exact Steam/thread/start-time checks and an unconditional restore guard are host-tested. | Advance only after an excluded native-resolution Low run reaches a game-authored result, restores CPUs0-3, preserves Steam/X11, and reduces performance-core contention. Then use the cooled ABBA wrapper. |
