...
| BVB descriptor allocation reuse | E128 removed opcode-123 socket exchanges, but its per-allocation futex/worker path regressed average FPS from 2.3 to 2.0 and raised descriptor blocking from 102.1 to 141.0 ms/present. PRoot is absent from the live game's parent chain. | Profile worker scheduling versus native allocation, then virtualize descriptor-pool reset epochs and recycle native sets. Moving the same roughly 531 synchronous allocations to another transport is rejected. |
