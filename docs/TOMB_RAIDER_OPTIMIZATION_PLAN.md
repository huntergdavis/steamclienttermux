...
| Steam `IPC:CServiceEng` on CPU0 | Feasibility passed: the excluded profiled run completed at 33.7 FPS, restored CPUs0-3, and preserved Steam/X11. A separate `IPC:CSteamEngin` thread then led Steam at 48.3% CPU. | The profiler-free cooled ABBA series is in progress. Promote only if both isolated passes improve the matched controls without launch/input regressions; otherwise retain the exact machinery and pursue the newly exposed Steam thread. |
