...
| Bionic/system-Vulkan host | The bridge passes persistent frame handoffs, Steam loader selection, real Adreno limits, extension-enabled device creation, and base Vulkan 1.1 physical-device discovery. | Bridge measured Wine/DXVK instance/WSI extensions and extended feature chains, reach one real DXVK frame, then run the fixed 2800×1752 Low Tomb Raider A/B. |
