...
| Bionic/system-Vulkan host | Direct Android-loader enumeration, glibc/Bionic capability parity, and direct plus glibc-triggered native device/command submission now pass with zero buffer mismatches. | Continue as a separate architecture project. Next prove a controlled `ANativeWindow`, surface, swapchain, and deterministic presented frame; game-facing dispatch and DXVK integration still precede any Tomb Raider FPS A/B. |
