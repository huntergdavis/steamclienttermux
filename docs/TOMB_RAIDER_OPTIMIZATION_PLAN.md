...
| BVB descriptor allocation reuse | E130's bounded active handoff improved the complete native-resolution benchmark from 2.0 to 2.4 FPS. It cut client ring latency from 269.4 to 195.8 microseconds and completion wake from 25.0 to 3.5 microseconds, but the scene still performs about 529 synchronous descriptor transactions and blocks 103.6 ms per present. Native allocation is now 69% of the worker. | Remove the remaining per-set boundary. Preallocate/recycle same-layout native sets within authoritative descriptor-pool reset epochs and publish bounded typed leases; refill in batches and keep reset invalidation, finite capacity, native error semantics, and strict fallback. |
