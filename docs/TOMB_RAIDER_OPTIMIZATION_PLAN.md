...
| Bionic/system-Vulkan host | Direct enumeration, glibc/Bionic capability parity, native command submission, and a separately owned `ANativeWindow` Vulkan surface query now pass. | Continue as a separate architecture project. Next create the swapchain, render, acquire, and present a deterministic offscreen frame; a dedicated visible host, game-facing dispatch, and DXVK integration still precede any Tomb Raider FPS A/B. |
