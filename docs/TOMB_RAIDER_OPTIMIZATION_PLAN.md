...
| RakNet-exclusive CPU1 | The direct profile measured the single `Raknet-RecvFrom` thread at 99% on CPU1 while the game used 313% total. | One exact pair changed min/max/avg by +9.13%/+5.93%/-2.20%. Run the staged three-pair alternation; retain production unless the low-end gain repeats without an average regression. |
