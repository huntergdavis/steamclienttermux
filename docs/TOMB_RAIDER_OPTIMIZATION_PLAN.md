...
| RakNet empty-receive backoff | The retained syscall trace is the exact upstream empty-receive pattern: `RakSleep(0)` becomes Wine `NtDelayExecution(0)`, then `sched_yield`, while the receive thread consumes 99% CPU. | The exact-thread 1 ms shim reduced RakNet from 98.2% to 3.0% CPU and completed 32.9/31.3/30.7 FPS recorded passes (31.633 mean). That is +2.37% versus the retained immediate 30.9 control; keep it opt-in until the running same-session reverse control closes the variance question. |
