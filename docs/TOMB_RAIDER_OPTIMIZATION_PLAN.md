...
| BVB shared descriptor journal | E125 measured about 529 synchronous update-template RPCs and 72.9 ms blocked in them per benchmark-scene present. E126 batches 4,097 typed updates with zero update-time exchanges in its host proof. | `TOMB_RAIDER_BVB_DESCRIPTOR_JOURNAL=shared` is injected only into Wine/DXVK. Repeat the complete benchmark and E117 profile first; then attack the still-synchronous opcode-68 allocation family. |
