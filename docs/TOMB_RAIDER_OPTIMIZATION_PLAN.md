...
| Bionic/system-Vulkan host | Direct enumeration, glibc/Bionic parity, native commands, pixel-verified offscreen presentation, an immersive visible `NativeActivity`, and authenticated lifecycle/status handoff to glibc now pass. | Continue as a separate architecture project. Next trace the minimum Vulkan entry points used by DXVK startup and design generated handle/dispatch ownership; game-facing integration still precedes any Tomb Raider FPS A/B. |
