...
| BVB descriptor allocation reuse | E129 profiles 527,157 real transactions: the worker averages 78.2 microseconds, including 38.9 microseconds native allocation and 25.0 microseconds completion wake, while the client observes roughly 269.4 microseconds. Context-lock wait is only 0.6 microseconds. PRoot is absent from the live game's parent chain. | Remove both repeated native allocation and the per-set cross-process wait. Preallocate/recycle same-layout native sets within authoritative descriptor-pool reset epochs and publish bounded typed leases; keep reset invalidation and strict fallback. |
