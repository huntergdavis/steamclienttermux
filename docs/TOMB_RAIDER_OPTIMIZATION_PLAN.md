...
| Official DXVK 1.10.3 x32 app-local family | Valid but not promoted: 55.6 FPS, 6.9% above bundled but 5.9% below 2.4.1. | Preserve as a hash-pinned fallback and version-sensitivity datapoint. Do not spend another cooled pass unless a later driver/FEX change reverses the ordering. |
...
| FEX SMC checks `none` | Rejected for average throughput: 33.2 FPS versus accepted 34.0 (-2.35%), despite a 10.32% minimum-FPS-mean improvement and clean rendering/exit. | A material FEX invalidation change lands, or frame pacing becomes the primary target after average FPS is higher. |
| DXVK 1.10.3 x32 | Closed behind 2.4.1: one controlled pass averaged 55.6 FPS versus 59.1 for 2.4.1. It still beat bundled DXVK's 52.0. | A new Turnip/FEX release or a game-specific regression in 2.4.1 changes the compatibility/performance ordering. |
