...
| BVB shared mapped memory | The E077 host selector independently stages the bridge's mapped-memory fast path without weakening the strict default or exposing its control to Steam/CEF and probe helpers. | After the matching bridge capability lands and passes visible-frame correctness, compare `TOMB_RAIDER_BVB_MAPPED_MEMORY=strict` (default) with `shared`, crossed independently with the command-stream selector in cooled native-resolution runs. |
