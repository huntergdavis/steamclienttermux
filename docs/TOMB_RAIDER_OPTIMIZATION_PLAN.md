...
| DXVK relaxed graphics UAV barriers | The post-FEX excluded profile measured Tomb Raider at 230.9% CPU, DXVK's command thread at 20.4%, and GPU busy at 74% while clocked at 818 MHz. Pinned DXVK supports a graphics-only relaxation that leaves compute barriers strict. | Run one excluded correctness/profile pass first. If rendering and exit remain correct, run the cooled three-pass candidate followed by an immediate control; reject on visual corruption or non-repeatable FPS. |
