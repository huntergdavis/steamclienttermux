...
| RakNet empty-receive backoff | The retained syscall trace is the exact upstream empty-receive pattern: `RakSleep(0)` becomes Wine `NtDelayExecution(0)`, then `sched_yield`, while the receive thread consumes 99% CPU. | Opt-in only: replace `sched_yield` with a real 1 ms sleep on the exact `Raknet-RecvFrom` thread. Do not skip `recvfrom`, which would break packet delivery; do not make the yield a no-op, which would spin faster. First require thread-CPU and game-connectivity proof, then run a cooled paired FPS series. |
