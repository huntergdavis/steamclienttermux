...
| RakNet-exclusive CPU1 | The direct profile measured the single `Raknet-RecvFrom` thread at 99% on CPU1 while the game used 313% total. | Pair production game CPUs1-7 against CPUs2-7 with RakNet alone on CPU1. Retain production unless exact ready guards and alternating replication show a repeatable gain. |
