...
TOOL = REPO_ROOT / "scripts" / "configure-tombraider-performance.py"


def load_tool():
    spec = importlib.util.spec_from_file_location("configure_tombraider_performance", TOOL)
...
def test_refusals(module):
    ...
    try:
        ...
        assert "exactly one Tomb Raider graphics registry section" in str(error)
    else:
    ...
    duplicate_key = original.replace(
        module.SECTION + b" 2\r\n", module.SECTION + b" 2\r\n" + key_line
    )
    try:
...
def test_process_guard(module, temporary):
    ...
    (process / "comm").write_text("TombRaider.exe\n")
    (process / "cmdline").write_bytes(b"Z:\\game\\TombRaider.exe\0")
    matches = module.find_running_prefix_processes(proc_root)
    assert [(pid, comm) for pid, comm, _cmdline in matches] == [(123, "TombRaider.exe")]
...
def main():
    ...
    print("Tomb Raider performance profile tests: PASS")
    return 0
