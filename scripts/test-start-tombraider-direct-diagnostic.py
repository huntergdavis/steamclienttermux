...
SCRIPT = Path(__file__).with_name("start-tombraider-direct-diagnostic.sh")


def main() -> None:
    with tempfile.TemporaryDirectory(prefix="tombraider-direct-diagnostic.") as directory:
        root = Path(directory)
        result = root / "result"
    ...
    print("Tomb Raider direct diagnostic-wrapper tests: PASS")
