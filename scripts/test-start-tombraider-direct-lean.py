...
SCRIPT = Path(__file__).with_name("start-tombraider-direct-lean.sh")


def main() -> None:
    with tempfile.TemporaryDirectory(prefix="tombraider-direct-lean.") as directory:
        root = Path(directory)
        result = root / "result"
    ...
    print("Tomb Raider direct lean-wrapper tests: PASS")
