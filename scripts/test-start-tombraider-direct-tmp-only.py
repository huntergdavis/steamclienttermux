...
SCRIPT = Path(__file__).with_name("start-tombraider-direct-tmp-only.sh")


def main() -> None:
    with tempfile.TemporaryDirectory(prefix="tombraider-direct-tmp-only.") as directory:
        root = Path(directory)
        result = root / "result"
    ...
    print("Tomb Raider direct tmp-only wrapper tests: PASS")
