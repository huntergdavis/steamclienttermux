...
if [[ ! -f "$benchmark_runner" || -L "$benchmark_runner" || ! -x "$benchmark_runner" ]]; then
    printf '1080p Ultra shadow-isolation benchmark runner is unavailable or unsafe: %s\n' "$benchmark_runner" >&2
    exit 1
fi
...
printf 'Starting Tomb Raider BVB probe: workload=glibc-dxvk-241-x32-1080p-ultra-no-tessellation-ssao1-dof1-shadow1-benchmark\n'
"$benchmark_python" "$benchmark_runner" \
    --backend direct --profile safe \
    --game-profile 1080p-ultra-no-tessellation-ssao1-dof1-shadow1 \
    --fex-code-cache compiled --dxvk-variant dxvk-2.4.1-x32 \
    --startup-topology full --start-temperature-ceiling-c 40 \
    --warmups 0 --runs 1 "$@"
