...
if [[ ! -f "$benchmark_runner" || -L "$benchmark_runner" || ! -x "$benchmark_runner" ]]; then
    printf '720p High benchmark runner is unavailable or unsafe: %s\n' "$benchmark_runner" >&2
    exit 1
fi
...
printf 'Starting Tomb Raider BVB probe: workload=glibc-dxvk-241-x32-720p-high-benchmark\n'
"$benchmark_python" "$benchmark_runner" \
    --backend direct \
    --profile safe \
    --game-profile 720p-high \
