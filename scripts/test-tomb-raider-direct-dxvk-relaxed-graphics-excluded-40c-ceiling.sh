...
[[ -f $benchmark_runner && ! -L $benchmark_runner && -x $benchmark_runner ]] || {
    printf 'test-tomb-raider-direct-dxvk-relaxed-graphics-excluded-40c-ceiling: benchmark runner is unavailable or unsafe: %s\n' \
        "$benchmark_runner" >&2
    exit 1
}
...
exec "$benchmark_runner" \
    ...
    --profile safe \
    --fex-code-cache on \
    --dxvk-relaxed-graphics-barriers on \
