...
[[ -f $benchmark_runner && ! -L $benchmark_runner && -x $benchmark_runner ]] || {
    printf 'test-tomb-raider-direct-safe-full-topology-raknet-exclusive-pair-40c-ceiling: benchmark runner is unavailable or unsafe: %s\n' \
        "$benchmark_runner" >&2
    exit 1
}
...
exec "$benchmark_runner" \
    ...
    --runs 2 \
    --raknet-exclusive-recorded-passes 2 \
